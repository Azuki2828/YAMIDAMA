...
#include "AABB.h"
	...
	public:
		...
		void AddLeaf(const CVector3& position, void* extraData) 
		{
			// リーフを作る。
		...
		void WalkTree(const CVector3& pos, std::function<void(SLeaf* leaf)> onEndWalk);
	private:
		/// <summary>
		/// 平面でリーフノードを分割していく
		/// </summary>
		...
		/// <returns></returns>
		CVector3 CalcCenterPositionFromLeafList(
			const std::vector<SEntityPtr>& leafArray
		);
		/// <summary>
		/// 新しいBSPツリーの要素を作成する。
		...
		void WalkTree(SEntityPtr entity, const CVector3& pos, std::function<void(SLeaf* leaf)> onEndWalk );
	};