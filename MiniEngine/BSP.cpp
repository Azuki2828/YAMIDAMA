...
#include "BSP.h"
    ...
    CVector3 BSP::CalcCenterPositionFromLeafList(const std::vector<SEntityPtr>& leafArray)
    {
        // まずは、AABBの中心座標を求める。
        CVector3 centerPos;
    ...
    BSP::SEntityPtr BSP::CreateBSPTreeEntity(const std::vector<SEntityPtr>& leafArray)
    {
        ...
        CVector3 centerPos = CalcCenterPositionFromLeafList(leafArray);
        ...
        // 各共分散の要素を引っ張ってくる。
        CVector3* v_0 = (CVector3*)covarianceMatrix[0];
        CVector3* v_1 = (CVector3*)covarianceMatrix[1];
        CVector3* v_2 = (CVector3*)covarianceMatrix[2];
    ...
    void BSP::WalkTree(SEntityPtr entityPtr, const CVector3& pos, std::function<void(SLeaf* leaf)> onEndWalk)
    {
    ...
    void BSP::WalkTree(const CVector3& pos, std::function<void(SLeaf* leaf)> onEndWalk)
    {
