...
{
	m_constantBuffer.Init(sizeof(CMatrix));
...
void DebugWireframe::ConstantBufferUpdate()
{
	...
	//ビューとプロジェクションの掛け算
	CMatrix v = g_camera3D->GetViewMatrix();
	CMatrix p = g_camera3D->GetProjectionMatrix();
	VP.Multiply(v, p);
...
void DebugWireframe::RenderContextUpdate(CRenderContext& rc)
{
	//ルートシグネチャを設定。
...
void DebugWireframe::End(CRenderContext& rc)
{
